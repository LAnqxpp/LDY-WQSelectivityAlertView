...
@property (nonatomic, strong) NSIndexPath *isIndexFirst;//是否默认选中第一条数据
...
-(instancetype)initWithTitle:(NSString *)title
                       datas:(NSArray *)datas
           ifSupportMultiple:(BOOL)ifSupportMultiple isIndexFirst:(NSIndexPath *)isIndexFirst btns:(BOOL )btns {
    if (self = [super init]) {
        ...
        self.alertView = [[UIView alloc] initWithFrame:CGRectMake(50, (kScreen_Height-alertViewHeight)/2.0, kScreen_Width-100, alertViewHeight)];
        ...
        self.isIndexFirst = isIndexFirst;
        ...
        if (self.isIndexFirst.row == 0 && btns == YES) {
             self.confirmButton.frame = CGRectMake(0,CGRectGetMaxY(self.selectTableView.frame), self.alertView.frame.size.width, buttonHeight);
        }else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.ifSupportMultiple == NO) {
        self.selectIndexPath = self.isIndexFirst;
